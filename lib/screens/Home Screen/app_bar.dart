          ...
          child: PhysicalModel(
            ...
            child: AnimatedContainer(
              ...
              child: SafeArea(
                ...
                child: Stack(
                  children: [
                    // Animated recycling icon in the background
                    Positioned.fill(
                    ...
                    // Main content with crossfade between expanded and collapsed
                    Positioned.fill(
  ...
  Widget _buildCollapsedAppBar(BuildContext context, double animationProgress) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          // App name with scaling and centering
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Top row with user greeting and actions
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              // User greeting
          ...
          // Stats row with bouncy entrance animation
          TweenAnimationBuilder<double>(
...
// Reusable AvatarButton
class AvatarButton extends StatelessWidget {
  final VoidCallback onPressed;
  final String userAvatarUrl;
  final bool small;

  const AvatarButton({Key? key, required this.onPressed, this.userAvatarUrl = '', this.small = false}) : super(key: key);
      ...
      child: Container(
        ...
        child: Hero(
          ...
          child: CircleAvatar(
            ...
            child: Center(child: Icon(Icons.person, size: 36)), // Fallback icon
          ),
        )
      ),
    );
...
class InfoChip extends StatelessWidget {
  ...
  const InfoChip({Key? key, required this.icon, required this.text, required this.textColor, required this.bgColor}) : super(key: key);
...
class EcoPatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Draw scattered leaves with random rotations
    ...
    // Draw improved circular recycling symbol
    final center = Offset(size.width * 0.5, size.height * 0.6);
    final radius = size.width * 0.08;
    final arrowPath = Path();

    // Draw three circular arrows with improved design
    for (int i = 0; i < 3; i++) {
      ...
      // Add arrow tip with improved design
...
class RecyclingIconPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Draw rotating recycling icon
    canvas.save();
    canvas.translate(center.dx, center.dy);
    canvas.rotate(progress * 2 * pi); // Rotate based on animation progress
    canvas.translate(-center.dx, -center.dy);
